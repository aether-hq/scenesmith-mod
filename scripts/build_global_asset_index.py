...
def _catalog_rows(
    *,
    ...
    memberships = _category_memberships(
        preprocessed_root / "object_categories.json"
    )
...
def _hssd_rows(
    *,
    ...
    index = json.loads(
        (preprocessed_root / "hssd_wnsynsetkey_index.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    for synset, entries in sorted(index.items()):
        ...
        for raw in entries:
            ...
            row = _normalized_row(
                    ...
                    thumbnail=None,
                    mesh_path=(
                        data_root / "objects" / source_id[0] / f"{source_id}.glb"
                    ),
                    embedding_row=embedding_offset + len(rows),
                )
            rows.append(row)
...
def build_global_index(
    *,
    ...
    categories = {
        category: [
            row["uid"] for row in rows if category in row["placement_classes"]
        ]
