...
async def _settle_cancelled_workflow(
    task: asyncio.Task[Any],
    *,
    monitor_cancel_started: bool,
) -> None:
    """Cancel an SDK task and any CLI fallback routes it starts while unwinding."""

    cleanup_seconds = float(
        os.environ.get("SCENESMITH_AGENT_CANCELLATION_GRACE_SECONDS", "8")
    )
    if cleanup_seconds <= 0 or cleanup_seconds > 30:
        raise ValueError(
            "SCENESMITH_AGENT_CANCELLATION_GRACE_SECONDS must be from 0 to 30"
        )
    task.cancel()
    if not monitor_cancel_started:
    ...
    while not task.done() and asyncio.get_running_loop().time() < deadline:
        ...
        if done:
            break
        # A cancelled CLI request can return an HTTP error that advances the
        # harness to its next fallback before the outer task observes cancellation.
        # Cancel that new route too instead of waiting out another full deadline.
        await asyncio.to_thread(cancel_subscription_turn)
        task.cancel()
    if task.done():
    ...
    console_logger.error(
        "Timed-out agent workflow did not settle within %.1fs cancellation grace",
        cleanup_seconds,
    )
...
class BoundedRunner:
    ...
    async def run(*args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            # thread. Terminate the current CLI route before awaiting the SDK task,
            # and keep cancelling any fallback route started while it unwinds.
            await _settle_cancelled_workflow(
                task,
                monitor_cancel_started=monitor_cancel_started.is_set(),
            )
