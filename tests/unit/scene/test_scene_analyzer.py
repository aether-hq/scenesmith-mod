...
def _support_surface(object_id: str, elevation: float):
    return SimpleNamespace(
        ...
        transform=SimpleNamespace(translation=lambda: np.array([0.0, 0.0, elevation])),
    )
...
def test_collection_library_details_use_blueprint_style_across_levels(tmp_path):
    ...
    assert all(
        "thousands of books" in selection.prompt_constraints for selection in selections
    )
    assert all(
        "dense rows of visible" in selection.suggested_items for selection in selections
    )
...
def test_ordinary_room_keeps_configured_sparse_surface_limit(tmp_path):
    objects = {
        surface.object_id: surface
        for surface in (
            _support_surface(f"bookcase_{index}", 0.0) for index in range(6)
        )
    }
    scene = SimpleNamespace(
        scene_dir=tmp_path,
