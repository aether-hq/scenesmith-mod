...
class TestPrisonEscapeExample(unittest.TestCase):
    def test_clean_rebuild_is_atomic_and_records_compiler_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            source = Path(__file__).parents[3] / manifest["build"]["source_path"]
    ...
    def test_web_viewer_references_generated_architecture(self) -> None:
        example_dir = Path(__file__).parents[3] / "examples" / "prison_escape"
    ...
    def test_checked_in_demo_manifest_references_current_content_addressed_bundle(
        ...
        generated = (
            Path(__file__).parents[3] / "examples" / "prison_escape" / "generated"
        )
