...
def test_large_locked_groups_become_exact_surviving_geometry(tmp_path: Path):
    ...
    bay_sdf = bays[0].sdf_path.read_text(encoding="utf-8")
    assert "<static>" not in bay_sdf
    assert "service_pad_visual" in bay_sdf
    assert "service_pad_collision" not in bay_sdf
