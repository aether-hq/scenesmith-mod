...
class TestSamProvider(unittest.TestCase):
    ...
    def test_mlx_memory_policy_is_bounded_and_provider_owned(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            with (
                patch(
                    "scenesmith.agent_utils.geometry_generation_server.pipelines.sam_provider.validate_sam_provider_config"
                ),
                patch(
                    "scenesmith.agent_utils.geometry_generation_server.pipelines.sam_provider._create_foreground_mask"
                ),
                patch(
                    "scenesmith.agent_utils.geometry_generation_server.pipelines.sam_provider.subprocess.run",
            ...
            self.assertLess(
                float(run.call_args.kwargs["env"]["PYTORCH_MPS_LOW_WATERMARK_RATIO"]),
                0.75,
            )
            ...
            self.assertEqual(run.call_args.kwargs["env"]["SPARSE_ATTN_BACKEND"], "sdpa")
