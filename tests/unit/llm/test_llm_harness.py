...
from scenesmith.agent_utils.llm import llm_harness as llm_harness_module
...
from scenesmith.agent_utils.scene.manipuland_context import (
    deterministic_manipuland_assignment,
)
...
def test_timeout_retries_then_uses_the_next_model_without_opening_circuit():
    ...
    harness = LLMHarness(cli_config(attempts=2, fallbacks=("opus",)), adapter=adapter)
...
def test_agents_sdk_path_retries_cancels_orphans_and_fails_over_models(monkeypatch):
    ...
    provider = HardenedModelProvider(raw, cli_config(attempts=2, fallbacks=("opus",)))
...
def test_medical_bed_assignment_excludes_hard_equipment():
    obj = type("Object", (), {"name": "medical bed", "description": "treatment bed"})()
